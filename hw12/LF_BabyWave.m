% LF_BabyWave solves the baby wave equation Ut + aUx = 0 by Lax-Friedrichs
% scheme.
...
function u = LF_BabyWave(h, k, a, x, f, T)
Nk = T / k;
Nx = length(x);
sigma = a * k / h;
e = ones(Nx, 1);
A = spdiags([0.5 * (1 + sigma) * e, zeros(Nx, 1), ...
    0.5 * (1 - sigma) * e], -1 : 1, Nx, Nx);
u = f;
for kk = 1 : Nk
    u = A * u;
end
